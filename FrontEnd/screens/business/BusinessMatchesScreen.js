// FrontEnd/screens/business/BusinessMatchesScreen.js
// Business matches: every candidate the business has mutually matched with.
// Card layout focused on the candidate; conversations live in the Messages tab.
...
export default function BusinessMatchesScreen({ navigation }) {
  ...
  const fetchMatches = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      console.warn("BusinessMatchesScreen fetch error:", err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.msgBtn} onPress={() => openChat(item)} activeOpacity={0.85}>
          ...
          <Text style={styles.msgBtnText}>
            {item.last_message ? "Continue chat" : "Message candidate"}
          </Text>
        </TouchableOpacity>
      </View>
    );
      ...
      ListEmptyComponent={
        <View style={styles.empty}>
          ...
          <Text style={styles.emptySub}>
            Review your applicants and like the ones you want — mutual likes become matches.
          </Text>
          <TouchableOpacity
            style={styles.emptyBtn}
            onPress={() => navigation.navigate("Applicants")}
          >
            <Text style={styles.emptyBtnText}>Review Applicants</Text>
          </TouchableOpacity>
        </View>
      }
    />
